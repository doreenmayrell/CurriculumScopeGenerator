...
- CRITICAL — CODE SYSTEM: every lesson "code" must be a code from the UPLOADED standard system, NEVER a Common Core / CCSS code. TEKS codes are grade.knowledge.skill — "2.2.A", "K.9.B", "5.1.A" (a number then a letter). CCSS codes are grade.DOMAIN.CLUSTER.number — "2.NBT.A.1", "8.F.B.4". When the uploaded system is TEKS (or any non-CCSS system), it is wrong to label a lesson "2.NBT.A.1" — that is Common Core; CCSS is only the comparison baseline, never the lesson code. If the input does not give you the exact uploaded-system code, infer the correct uploaded-system expectation code for that grade (e.g. Texas place value to 1,200 is TEKS "2.2.A/2.2.B", not "2.NBT.A.1"); do not substitute a CCSS code.
...
- standard: the full standard text. baseCode: the parent standard's official code from the uploaded document. code (per new lesson): the most granular raw uploaded expectation exactly as written in the official document, with no + suffix and no proposed Substandard ID numbering (e.g. "K.2.A", "K.9.B", "5.1.A", "MA.K.NSO.1.3") — always the UPLOADED system's own code, never a CCSS code like "2.NBT.A.1". Do not return parent codes like "1.2" when the PDF contains children like "1.2.A". The app appends the Substandard ID numbering: for CCSS it forms CCSS.MATH.CONTENT.<<code>>+<<#>>; for other systems (e.g. TEKS) it forms <<SYSTEM>>.<<code>>+<<#>> (no MATH.CONTENT), choosing the next available number from the lesson library.
